...
const SERVER_URL_PRODUCTION = environment !== 'development' ? `${window.location.href.split('//')[1].split('/')[0]}/api` : '';
const SSL_PREFIX = environment !== 'development' && window.location.protocol == 'https:' ? 'https' : 'http';

export const SERVER_URL = environment !== 'development' ? `${SSL_PREFIX}/${SERVER_URL_PRODUCTION}` : 'http://localhost:5555/api';
